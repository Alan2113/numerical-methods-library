...
void testujMetodeBisekcji() {
    ...
    // Test 5: Test obs³ugi b³êdów - funkcja nie zmienia znaku
    std::cout << "\nTest 5: Obs³uga b³êdów" << std::endl;
    auto f5 = [](double x) { return x * x + 1; }; // Brak pierwiastków rzeczywistych
    try {
        BibliotekaNumeryczna::metodaBisekcji(f5, 0, 2, 0.001);
        std::cout << "? Test 5 nie przeszed³ - powinien rzuciæ wyj¹tek!" << std::endl;
        assert(false);
    }
    catch (const std::invalid_argument& e) {
        std::cout << "? Test 5 przeszed³ pomyœlnie - poprawnie rzucony wyj¹tek: " << e.what() << std::endl;
    }
